...
Then tell the user: "Already installed (since `<date from .installed>`). Pulled latest — your config is symlinked, so updates are live immediately. Run `/scan-apps` to check for new apps, or `/customize` to make changes." Stop here.
